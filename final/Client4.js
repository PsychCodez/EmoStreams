...
const app = express();
const port = 7004;  // Each client has a unique port
...
app.post('/data', (req, res) => {
  ...
  // For each emoji object in the feedback, print the emoji count times
  feedbackData.forEach((emoji) => {
    const emojiType = emoji.emoji_type;
    const count = emoji.count;

    // Create a string with the emoji repeated by its count
    const emojisToDisplay = emojiType.repeat(count);
    console.log('Displaying emojis:', emojisToDisplay);
  });
